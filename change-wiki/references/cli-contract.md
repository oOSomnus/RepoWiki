...
The global `--repo-root <repo>` selects the session-storage and lookup root independently of `--repo`. Pass it consistently to every session-based command when analyzing a temporary worktree or when session artifacts belong to another checkout. Session state keeps `repo_path` set to the analyzed repository.
